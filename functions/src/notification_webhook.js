...
exports.appStoreNotifications = onRequest({
  ...
  try {
    console.log('📡 App Store Server Notification 수신:', {
      method: req.method,
      headers: req.headers,
      body: req.body
    });
    ...
    const decodedRenewal = renewalInfo ? decodeJWS(renewalInfo) : null;
    ...
    // Firestore 업데이트 처리
    await processNotification(notificationType, subtype, decodedTransaction, decodedRenewal);
 ...
 */
async function processNotification(notificationType, subtype, transaction, renewal) {
  ...
  switch (notificationType) {
    case 'SUBSCRIBED':
      ...
      subscriptionUpdate = {
        ...
        status: 'active',
        startDate: admin.firestore.Timestamp.fromMillis(parseInt(transaction.purchaseDate)),
        expiryDate: admin.firestore.Timestamp.fromMillis(parseInt(transaction.expiresDate)),
        autoRenewStatus: renewal ? renewal.autoRenewStatus === 1 : true,
        isCancelled: false,
    ...
    case 'DID_CHANGE_RENEWAL_STATUS':
      // 자동 갱신 상태 변경
      const autoRenewStatus = renewal ? renewal.autoRenewStatus === 1 : false;
      subscriptionUpdate = {
        ...subscriptionUpdate,
        autoRenewStatus: autoRenewStatus,
        isCancelled: !autoRenewStatus, // 자동 갱신 꺼짐 = 취소
      };
      console.log(`🔄 자동 갱신 상태 변경: ${autoRenewStatus ? '활성화' : '비활성화'}`);
      break;
